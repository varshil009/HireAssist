...
candidates(
  ...
  current_stage INTEGER: -1=rejected, 0=applied, 1=screening, 2=offered, 3=hired,
...
Use datetime comparisons on entered_*_at columns (ISO-8601 strings).
...
def run_ai_search(query: str) -> SearchResponse:
    ...
    for attempt in range(2):
        try:
            ...
            return SearchResponse(
                ok=True,
                message=_format_rows_message(q, columns, serializable),
                query=q,
                columns=columns,
                rows=serializable,
                fuzzy_suggestions=fuzzy,
